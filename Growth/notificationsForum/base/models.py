...
from notificationsForum import settings as notificationsForum_settings
...
EXTRA_DATA = notificationsForum_settings.get_config()['USE_JSONFIELD']


def is_soft_delete():
    return notificationsForum_settings.get_config()['SOFT_DELETE']
...
class AbstractNotification(models.Model):
    ...
    recipient = models.ForeignKey(
        ...
        blank=False,
        related_name='notificationsForum',
        on_delete=models.CASCADE
    )
    unread = models.BooleanField(default=True, blank=False, db_index=True)

    actor_content_type = models.ForeignKey(ContentType, related_name='notify_actor_forum', on_delete=models.CASCADE)
    ...
    target_content_type = models.ForeignKey(
        ContentType,
        related_name='notify_target_forum',
        blank=True,
        null=True,
        on_delete=models.CASCADE
    )
    ...
    action_object_content_type = models.ForeignKey(ContentType, blank=True, null=True,
                                                   related_name='notify_action_object_forum', on_delete=models.CASCADE)
    ...
    class Meta:
        abstract = True
        ordering = ('-timestamp',)
        # speed up notificationsForum count query
...
def notify_handler(verb, **kwargs):
    """
    Handler function to create NotificationForum instance upon action signal call.
    """
    ...
    level = kwargs.pop('level', NotificationForum.LEVELS.info)
    ...
    new_notificationsForum = []

    for recipient in recipients:
        newnotify = NotificationForum(
            recipient=recipient,
        ...
        newnotify.save()
        new_notificationsForum.append(newnotify)

    return new_notificationsForum


# connect the signal
notify.connect(notify_handler, dispatch_uid='notificationsForum.models.NotificationForum')
