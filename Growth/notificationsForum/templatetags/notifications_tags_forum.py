''' Django notificationsForum template tags file '''
# -*- coding: utf-8 -*-
...
def notificationsForum_unread(context):
    user = user_context(context)
    if not user:
        return ''
    return user.notificationsForum.unread().count()


if StrictVersion(get_version()) >= StrictVersion('2.0'):
    notificationsForum_unread = register.simple_tag(takes_context=True)(notificationsForum_unread)  # pylint: disable=invalid-name
else:
    notificationsForum_unread = register.assignment_tag(takes_context=True)(notificationsForum_unread)  # noqa


@register.filter
def has_notification(user):
    if user:
        return user.notificationsForum.unread().exists()
    return False
...
def register_notify_callbacks(badge_class='live_notify_badge',  # pylint: disable=too-many-arguments,missing-docstring
    ...
    if api_name == 'list':
        api_url = reverse('notificationsForum:live_unread_notification_list')
    elif api_name == 'count':
        api_url = reverse('notificationsForum:live_unread_notification_count')
    else:
        return ""
    definitions = """
    ...
    """.format(
        ...
        api_url=api_url,
        unread_url=reverse('notificationsForum:unread'),
        mark_all_unread_url=reverse('notificationsForum:mark_all_as_read'),
        fetch_count=fetch
    )
...
def live_notify_badge(context, badge_class='live_notify_badge'):
    ...
    html = "<span class='{badge_class}'>{unread}</span>".format(
        badge_class=badge_class, unread=user.notificationsForum.unread().count()
    )
